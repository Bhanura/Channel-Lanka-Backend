...
(async () => {
  try {
    ...
    // Create a pending center
    const { data: center, error: centerErr } = await supabaseAdmin.from('channeling_centers').insert({
      name: 'Test Center',
    ...
    if (centerErr) return console.log('Failed to create center', centerErr);
